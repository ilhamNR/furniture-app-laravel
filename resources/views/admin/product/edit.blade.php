...
@section('content')
    ...
    <div id="kt_app_content_container" data-url={{ url('/') }} class="app-container container-xxl">
                ...
                <form id="add_product_form" class="form" action="{{ route('admin.saveProduct') }}" method="POST">
                    ...
                    <div class="card-body border-top p-9">
                        ...
                        <div class="row mb-6">
                            ...
                            <div class="col-lg-8">
                                <!--begin::Form-->
                                <input type="text" hidden id="thumbnail_data" data-value='{{ $thumbnail }}'
                                    name="thumbnail_data">
                        ...
                        <div class="row mb-6">
                            ...
                            <!--begin::Col-->
                            <div class="col-lg-8">
                                <input type="text" hidden id="photos_data" data-value='{{ $photos }}'
                                    name="photos_data" value="">
                                <!--begin::Form-->
                                <form class="form" action="#" method="post">
                                    <!--begin::Input group-->
                                    <div class="fv-row">
                            ...
                            <div class="col-lg-8 fv-row">
                                <select name="category" aria-label="Select a Category" data-control="select2"
                                    ...
                                    @foreach ($category as $item)
                                        @if ($item->id == $data->product_category_id)
...
@section('script')
    ...
    <script>
        ...
        var photosData = JSON.parse(photosInput.getAttribute('data-value'));

        // Get the element by its ID
        var thumbnailDataInput = document.getElementById('thumbnail_data');

        // Get the value of the data-value attribute
        var thumbnailDataValue = thumbnailDataInput.getAttribute('data-value');
        ...
        var productDropzone = new Dropzone("#photo_upload_zone", {
            ...
            init: function() {
                // Iterate through existing photosData and add them to Dropzone
                for (var i = 0; i < photosData.length; i++) {
                    var mock = photosData[i];
                    mock.url = baseUrl + '/storage/product_img/' + mock;
                    mock.accepted = true;
                    // console.log(mock);


                    // Trigger events to display existing files
                    this.emit("addedfile", mock.url);
                    this.emit("complete", mock.url);
            ...
            removedfile: function(file) {
                // Remove the file's server-generated filename from the array
                var index = photosArray.indexOf(file.serverFilename);
                if (index !== -1) {
